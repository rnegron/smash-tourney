# external dependencies for CLI usage : pydub, gTTS
...
def link_sound(tourney_dir: Path, temp_dir: Path, name_1: str, name_2: str) -> None:
    ...
    combined.export(str(tourney_dir / f"{name_1}_vs_{name_2}.mp3"), format="mp3")


def create_sound_google(temp_dir: Path, name: str) -> None:

    # gTTS greatly simplifies accessing Google's text-to-speech API
    tts = gTTS(text=name)
    temp_file = str(temp_dir / f"{name.replace(' ', '')}.mp3")
    tts.save(temp_file)


def cli(file_with_names: Path) -> None:
    ...
    if not versus_file.exists():
        logger.error(
            'No "versus.wav" file found in the directory! Please provide your own.'
        )
        sys.exit(1)

    tourney_dir = Path("./tourney")
    ...
    with file_with_names.open() as name_file:
        ...
        # Count iterations left to report them to the CLI
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        print(f"Usage: python {sys.argv[0]} file_with_names")
        sys.exit(1)
